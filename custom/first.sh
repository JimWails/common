...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/JimWails/common build/common
...
git clone -b main --depth 1 https://github.com/JimWails/build-actions shangyou
...
if [[ "${GIT_REPOSITORY}" =~ (JimWails/build-actions|JimWails/autobuild) ]]; then
  rm -rf backups
...
curl -fsSL https://raw.githubusercontent.com/JimWails/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/JimWails/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="JimWails/build-actions"
  sudo rm -rf build common.sh
